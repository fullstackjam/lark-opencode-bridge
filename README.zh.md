...
**确认第 6 步私聊 `/help` 正常后**，再开后台（macOS / Linux / Windows）：
...
lark-opencode-bridge start        安装并启动后台 daemon（macOS / Linux / Windows）
...
> 后台服务支持 **macOS（launchd）/ Linux（systemd --user）/ Windows（任务计划程序）**。`start` 等命令需 **全局安装**（`npm i -g`），勿用 npx。
...
│
├── service/
│   └── daemon.ts         # launchd（macOS）/ systemd（Linux）/ 任务计划程序（Windows）
│
└── media/
...
**Q：Windows 能用吗？**
能 —— 前台 `run` 和后台 daemon 都支持。Windows 上后台会注册一个**当前用户的「任务计划程序」任务**（登录时自启、崩溃自动重启），等价于 macOS 的 launchd、Linux 的 systemd `--user`。`start` / `stop` / `restart` / `status` 用法完全一致。
